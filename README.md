...
The video path decodes variable-frame-rate sources without duplicating frames.
To reduce pan ghosting, PyTorch inference and TensorRT temporal window engines
drop older frames from the temporal window when cumulative block-luma motion
exceeds `0.045`. Tune that threshold with `--motion-budget`, or use
`--disable-motion-budget` when you want to force the full causal window.
